...
def main():
    ...
    for label, data in zip(["train", "test"], [mnist_train, mnist_test]):
        ...
        while current < n_signals:
            ...
            # image_chw = np.transpose(chunk, (1, 2, 0))
            # cv2.imwrite("2.png", image_chw)
            #
            # image_chw = np.transpose(projections[idxs], (1, 2, 0))
            # cv2.imwrite("1.png", image_chw)
            # print(data['labels'][idxs])
            current += args.chunk_size
